export const images = {
    welcome: require('../img/iconSylva.png'),
    f: require('../img/f.png'),
    logo: require('../img/logo.png'),
    confirm: require('../img/confirm.png'),
    close: require('../img/close.png'),
    classicSuit: require('../img/classicSuit.png'),
    exoSuit: require('../img/exoSuit.png'),
    flightSuit: require('../img/flightSuit.png'),
    config: require('../img/iconConfig.png'),
    user: require('../img/user.png'),
    mail: require('../img/mail.png'),
    submit: require('../img/submit.png'),
    sylva: require('../img/sylva.png'),
    desolo: require('../img/desolo.png'),
    calidor: require('../img/calidor.png'),
    vesania: require('../img/vesania.png'),
    novus: require('../img/novus.png'),
    glacio: require('../img/glacio.png'),
    atrox: require('../img/atrox.png'),
    favIcon: require('../img/favIcon.png'),
    favIconActive: require('../img/favIconActive.png'),
    nuggetSylva: require('../img/iconSylva.png'),
    nuggetDesolo: require('../img/iconDesolo.png'),
    nuggetCalidor: require('../img/iconCalidor.png'),
    nuggetVesania: require('../img/iconVesania.png'),
    nuggetNovus: require('../img/iconNovus.png'),
    nuggetGlacio: require('../img/iconGlacio.png'),
    nuggetAtrox: require('../img/iconAtrox.png'),
    ammonium: require('../img/ammonium.png'),
    astronium: require('../img/astronium.png'),
    clay: require('../img/clay.png'),
    compound: require('../img/compound.png'),
    graphite: require('../img/graphite.png'),
    hematite: require('../img/hematite.png'),
    laterite: require('../img/laterite.png'),
    lithium: require('../img/lithium.png'),
    malachite: require('../img/malachite.png'),
    organic: require('../img/organic.png'),
    quartz: require('../img/quartz.png'),
    resin: require('../img/resin.png'),
    sphalerite: require('../img/sphalerite.png'),
    titanite: require('../img/titanite.png'),
    wolframite: require('../img/wolframite.png'),
    aluminum: require('../img/aluminum.png'),
    carbon: require('../img/carbon.png'),
    ceramic: require('../img/ceramic.png'),
    copper: require('../img/copper.png'),
    glass: require('../img/glass.png'),
    iron: require('../img/iron.png'),
    titanium: require('../img/titanium.png'),
    tungsten: require('../img/tungsten.png'),
    zinc: require('../img/zinc.png'),
    argon: require('../img/argon.png'),
    helium: require('../img/helium.png'),
    hydrogen: require('../img/hydrogen.png'),
    methane: require('../img/methane.png'),
    nitrogen: require('../img/nitrogen.png'),
    sulfur: require('../img/sulfur.png'),
    aluminumAlloy: require('../img/aluminumAlloy.png'),
    diamond: require('../img/diamond.png'),
    explosivePowder: require('../img/explosivePowder.png'),
    hydrazine: require('../img/hydrazine.png'),
    graphene: require('../img/graphene.png'),
    nanocarbonAlloy: require('../img/nanocarbonAlloy.png'),
    plastic: require('../img/plastic.png'),
    rubber: require('../img/rubber.png'),
    silicone: require('../img/silicone.png'),
    steel: require('../img/steel.png'),
    titaniumAlloy: require('../img/titaniumAlloy.png'),
    tungstenCarbide: require('../img/tungstenCarbide.png'),
    nuggetAmmonium: require('../img/nuggetAmmonium.png'),
    nuggetSphalerite: require('../img/nuggetSphalerite.png'),
    nuggetMalachite: require('../img/nuggetMalachite.png'),
    nuggetHydrogen: require('../img/nuggetHydrogen.png'),
    nuggetNitrogen: require('../img/nuggetNitrogen.png'),
    nuggetWolframite: require('../img/nuggetWolframite.png'),
    nuggetSulfur: require('../img/nuggetSulfur.png'),
    nuggetArgon: require('../img/nuggetArgon.png'),
    nuggetTitanite: require('../img/nuggetTitanite.png'),
    nuggetLithium: require('../img/nuggetLithium.png'),
    nuggetHematite: require('../img/nuggetHematite.png'),
    nuggetMethane: require('../img/nuggetMethane.png'),
    nuggetHelium: require('../img/nuggetHelium.png'),
    nuggetAstronium: require('../img/nuggetAstronium.png'),
    nuggetClay: require('../img/nuggetClay.png'),
    nuggetCompound: require('../img/nuggetCompound.png'),
    nuggetGraphite: require('../img/nuggetGraphite.png'),
    nuggetLaterite: require('../img/nuggetLaterite.png'),
    nuggetOrganic: require('../img/nuggetOrganic.png'),
    nuggetQuartz: require('../img/nuggetQuartz.png'),
    nuggetResin: require('../img/nuggetResin.png'),
    nuggetAluminum: require('../img/nuggetAluminum.png'),
    nuggetCeramic: require('../img/nuggetCeramic.png'),
    nuggetCopper: require('../img/nuggetCopper.png'),
    nuggetGlass: require('../img/nuggetGlass.png'),
    nuggetIron: require('../img/nuggetIron.png'),
    nuggetTitanium: require('../img/nuggetTitanium.png'),
    nuggetTungsten: require('../img/nuggetTungsten.png'),
    nuggetZinc: require('../img/nuggetZinc.png'),
    nuggetAluminumAlloy: require('../img/nuggetAluminumAlloy.png'),
    nuggetDiamond: require('../img/nuggetDiamond.png'),
    nuggetExplosivePowder: require('../img/nuggetExplosivePowder.png'),
    nuggetHydrazine: require('../img/nuggetHydrazine.png'),
    nuggetGraphene: require('../img/nuggetGraphene.png'),
    nuggetNanocarbonAlloy: require('../img/nuggetNanocarbonAlloy.png'),
    nuggetPlastic: require('../img/nuggetPlastic.png'),
    nuggetRubber: require('../img/nuggetRubber.png'),
    nuggetSilicone: require('../img/nuggetSilicone.png'),
    nuggetSteel: require('../img/nuggetSteel.png'),
    nuggetTitaniumAlloy: require('../img/nuggetTitaniumAlloy.png'),
    nuggetTungstenCarbide: require('../img/nuggetTungstenCarbide.png'),
    nuggetExoChip: require('../img/nuggetExoChip.png'),
    nuggetScrap: require('../img/nuggetScrap.png'),
    nuggetSmallCanister: require('../img/nuggetSmallCanister.png'),
    nuggetSmallCanisterFull: require('../img/nuggetSmallCanisterFull.png'),
    soilCentrifuge: require('../img/soilCentrifuge.png'),
    tradePlatform: require('../img/tradePlatform.png'),
    smallPrinter: require('../img/smallPrinter.png'),
    chemistryLab: require('../img/chemistryLab.png'),
    thruster: require('../img/thruster.png'),
    exoChip: require('../img/exoChip.png'),
    packager: require('../img/packager.png'),
    dynamite: require('../img/dynamite.png'),
    smeltingFurnace: require('../img/smeltingFurnace.png'),
    backpackPrinter: require('../img/backpackPrinter.png'),
    generator: require('../img/generator.png'),
    tether: require('../img/tether.png'),
    roverSeat: require('../img/roverSeat.png'),
    mediumPrinter: require('../img/mediumPrinter.png'),
    trailer: require('../img/rover.png'),
    research: require('../img/research.png'),
    largePrinter: require('../img/largePrinter.png'),
    splitter: require('../img/splitter.png'),
    crane: require('../img/crane.png'),
    battery: require('../img/battery.png'),
    worklight: require('../img/worklight.png'),
    beacon: require('../img/beacon.png'),
    sensor: require('../img/sensor.png'),
    canister: require('../img/canister.png'),
    oxygenTank: require('../img/oxygenTank.png'),
    storage: require('../img/storage.png'),
    platform: require('../img/platform.png'),
    oxygenator: require('../img/oxygenator.png'),
    windTurbine: require('../img/windTurbine.png'),
    shuttle: require('../img/shuttle.png'),
    landingPad: require('../img/landingPad.png'),
    nuggetCarbon: require('../img/nuggetCarbon.png'),
    augment: require('../img/augment.png'),
    drill: require('../img/drill.png'),
    solar: require('../img/solar.png'),
    extenders: require('../img/extenders.png'),
    atmosphericCondenser: require('../img/atmosphericCondenser.png'),
    shredder: require('../img/shredder.png'),
    powerCells: require('../img/powerCells.png'),
    paver: require('../img/paver.png'),
    recreationalSphere: require('../img/recreationalSphere.png'),
    fireworks: require('../img/fireworks.png'),
    shelter: require('../img/shelter.png'),
    figurine: require('../img/figurine.png'),
    winch: require('../img/winch.png'),
    autoExtractor: require('../img/autoExtractor.png'),
    iconSmallPrint: require('../img/iconSmallPrint.png'),
    backpack: require('../img/iconBackpack.png'),
    iconMediumPrint: require('../img/iconMediumPrint.png'),
    iconLargePrint: require('../img/iconLargePrint.png'),
    iconSmeltingFurnace: require('../img/iconSmeltingFurnace.png'),
    iconSoilCentrifuge: require('../img/iconSoilCentrifuge.png'),
    iconChemistryLab: require('../img/iconChemistryLab.png'),
    iconAthmospheticCondenserLab: require('../img/iconAthmospheticCondenserLab.png'),
    iconTradePlatform: require('../img/iconTradePlatform.png'),
    iconExoRequestPlatform: require('../img/iconExoRequestPlatform.png'),
    iconRailEngine: require('../img/iconRailEngine.png'),
    iconRailCar: require('../img/iconRailCar.png'),
    iconRailStation: require('../img/iconRailStation.png'),
    iconRailPost: require('../img/iconRailPost.png'),
    iconRailPostBundle: require('../img/iconRailPostBundle.png'),
    iconTallRailPost: require('../img/iconTallRailPost.png'),
    iconTallRailPostBundle: require('../img/iconTallRailPostBundle.png'),
    iconRailJunction: require('../img/iconRailJunction.png'),
    iconRailJunctionBundle: require('../img/iconRailJunctionBundle.png'),
    iconCole: require('../img/iconCole..png'),
    iconSitePylon: require('../img/iconSitePylon.png'),
    probeScanner: require('../img/probeScanner.png'),
    scrap: require('../img/scrap.png'),
    railEngine: require('../img/railEngine.png'),
    railStation: require('../img/railStation.png'),
    railPost: require('../img/railPost.png'),
    bouncevine: require('../img/bouncevine.png'),
    bouncevineSeed: require('../img/bouncevineSeed.png'),
    popcoral: require('../img/popcoral.png'),
    popcoralSeed: require('../img/popcoralSeed.png'),
    daggeroot: require('../img/daggeroot.png'),
    daggerootSeed: require('../img/daggerootSeed.png'),
    hissbine: require('../img/hissbine.png'),
    hissbineSeed: require('../img/hissbineSeed.png'),
    lashleaf: require('../img/lashleaf.png'),
    lashleafSeed: require('../img/lashleafSeed.png'),
    volatileCataplant: require('../img/volatileCataplant.png'),
    volatileCataplantSeed: require('../img/volatileCataplantSeed.png'),
    thistlewhip: require('../img/thistlewhip.png'),
    thistlewhipSeed: require('../img/thistlewhipSeed.png'),
    volatileAttactus: require('../img/volatileAttactus.png'),
    volatileAttactusSeed: require('../img/volatileAttactusSeed.png'),
    elegantSpewflower: require('../img/elegantSpewflower.png'),
    elegantSpewflowerSeed: require('../img/elegantSpewflowerSeed.png'),
    wheezeweeds: require('../img/wheezeweed.png'),
    wheezeweedsSeed: require('../img/wheezeweedSeed.png'),
    noxiousCataplant: require('../img/noxiousCataplant.png'),
    noxiousCataplantSeed: require('../img/noxiousCataplantSeed.png'),
    spinyAttactus: require('../img/spinyAttactus.png'),
    spinyAttactusSeed: require('../img/spinyAttactusSeed.png'),
    spinelily: require('../img/spinelily.png'),
    spinelilySeed: require('../img/spinelilySeed.png'),
    noxiousSpewflower: require('../img/noxiousSpewflower.png'),
    noxiousSpewflowerSeed: require('../img/noxiousSpewflowerSeed.png'),
    boomalloon: require('../img/boomalloon.png'),
    boomalloonSeed: require('../img/boomalloonSeed.png')
}